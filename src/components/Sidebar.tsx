...
const Sidebar = ({
  ...
  const directionClass = () => {
    if (direction === 'ltr') {
      ...
      if (slideFromHeader) {
        return isOpen ? 'top-[64px] left-0 w-full h-[300px]' : 'top-[64px] left-0 -translate-y-[364px] w-full h-[300px]'
      }
  ...
  return (
    <div ref={sidebarRef} className={twMerge(`z-50 w-[350px] fixed border-neutral-100 border shadow-lg transition-all ease-in-out duration-700 ${directionClass()} ${className}`)}>
      {children}
    </div>
  )
}
